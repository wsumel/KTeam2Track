...
from lib.models.mdtrack.MoE_fusion import BiMixtureOfAdapters
...
class InteractionBlock(nn.Module):
    ...
    def forward(self,x,xs,xxs,blocks):
        x = self.injector(x.permute(1,0,2),xs.permute(1,0,2)).permute(1,0,2)
        x = self.injector(x.permute(1,0,2),xxs.permute(1,0,2)).permute(1,0,2)
        ...
        xxs = checkpoint.checkpoint(self.extractor, xxs.permute(1,0,2),x.permute(1,0,2),use_reentrant=False).permute(1,0,2) \
            if self.grad_ckpt else self.extractor(xxs.permute(1, 0, 2), x.permute(1, 0, 2)).permute(1, 0, 2)  # b,n,c
        ...
        if self.extra_extractors is not None:
            ...
            for extractor in self.extra_extractors:
                xxs = checkpoint.checkpoint(extractor, xxs.permute(1, 0, 2), x.permute(1, 0, 2), use_reentrant=False).permute(1, 0, 2) \
                    if self.grad_ckpt else extractor(xxs.permute(1, 0, 2), x.permute(1, 0, 2)).permute(1, 0,2)  # b,n,c
...
class Mamba_Neck(nn.Module):
    def __init__(self, in_channel=512,d_model=512,d_inner=1024,bias=False,n_layers=4,dt_rank=32,d_state=16,d_conv=3,dt_min=0.001,
        ...
        self.cross_injector = Injector(d_model=d_model)

    def forward(self, x, xs, xxs, h, xh, blocks, interaction_indexes):
        #  x : (B, L, D)
        ...
        for i,index in enumerate(interaction_indexes):
            ...
            xxs, xh[i] = self.layers[i](xxs, xh[i])
            xs = self.cross_injector(xs.permute(1,0,2),xxs.permute(1,0,2)).permute(1,0,2)
            xxs = self.cross_injector(xxs.permute(1,0,2),xs.permute(1,0,2)).permute(1,0,2)
            x,xs,xxs = self.interactions[i](x,xs,xxs,blocks[index[0]:index[1]])

        return x, xs, xxs, h, xh
...
if __name__ == "__main__":
    ...
    # xs = img_patch = torch.randn(1, 196, 512)
    # xxs = img_patch = torch.randn(1, 196, 512)
    # task_index = [0]
    # model = BiMixtureOfAdapters(dim=512,r=16,task_num=3)
    # # transfer to device
    ...
    # # encoder
    # macs1, params1 = profile(model, inputs=(xs,xxs,task_index), custom_ops=custom_ops ,verbose=True)
    ...
    #     # overall
    #     for i in range(T_w):
    #         _ = model(xs,xxs,task_index)
    #     start = time.time()
    #     for i in range(T_t):
    #         _ = model(xs,xxs,task_index)
    #     end = time.time()
    #     avg_lat = (end - start) / (T_t * 1)
    #     print("The average overall latency is %.2f ms" % (avg_lat * 1000))

    
    xs = img_patch = torch.randn(1, 196, 512)
    h = img_patch = torch.randn(1, 196, 1024,16)


    model = ResidualBlock(1.0, 512, 1024, 32, 16, False, 3, True, 'random', 0.1, 0.001, 0.0001, True)
    ...
    custom_ops = {nn.MultiheadAttention: get_complexity_MHA}
    # encoder
    macs1, params1 = profile(model, inputs=(xs,h), custom_ops=custom_ops ,verbose=True)
    macs, params = clever_format([macs1*8, params1*8], "%.3f")
    print('encoder macs is ', macs)
    print('encoder params is ', params)
    ...
    with torch.no_grad():
        ...
        avg_lat = (end - start) / (T_t * 1)
        print("The average overall latency is %.2f ms" % (avg_lat * 8000))
