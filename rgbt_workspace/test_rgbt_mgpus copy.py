...
def genConfig(seq_path, set_type):
    ...
    if set_type == 'RGBT234':
        ...
        with open("/disk3/wsl_tmp/Workspace210/Modality-missing-RGBT-Tracking-main/RGBT234-Miss/missing_results_rgbt234.json",'r') as load_f:
            mask_dict = json.load(load_f)            
            seq_mask_list = mask_dict
    ...
    return RGB_img_list, T_img_list, RGB_gt, T_gt,seq_mask_list
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='prompt'):
    ...
    save_path = f'/disk3/wsl_tmp/Workspace210/MDTrack/results/{dataset_name}/{save_name}_{epoch}_2/' + seq_txt + '.txt'

    save_folder = f'/disk3/wsl_tmp/Workspace210/MDTrack/results/{dataset_name}/{save_name}_{epoch}_2/'
    ...
    RGB_img_list, T_img_list, RGB_gt, T_gt, seq_mask_list = genConfig(seq_path, dataset_name)
    ...
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            ...
            # track
            image = get_x_frame(rgb_path, T_path, dtype=getattr(params.cfg.DATA,'XTYPE','rgbrgb'))
            region, confidence = tracker.track(image,seq_mask_list[seq_name]['data'][frame_idx])  # xywh
...
class MDTrack_RGBT(object):
    ...
    def track(self, img_RGB, seq_mask_list):
        '''TRACK'''
        # print(img_RGB.shape)
        # print(img_RGB.sum())
        if  seq_mask_list[0] == 0.0 and seq_mask_list[1] == 0.0:
            return -1,-1
        if  seq_mask_list[0] == 0.0 and seq_mask_list[1] == 1.0: 
            img_RGB[:,:,:3] = img_RGB[:,:,3:6].copy()
        if  seq_mask_list[0] == 1.0 and seq_mask_list[1] == 0.0: 
            img_RGB[:,:,3:6] = img_RGB[:,:,:3].copy()
...
if __name__ == '__main__':
    ...
    if dataset_name == 'GTOT':
        seq_home = '/nasdata/tracking/data/GTOT'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'RGBT234':
        seq_home = '/disk3/dataset/RGBT234/RGBT234'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'LasHeR':
        seq_home = '/disk3/wsl_tmp/Workspace210/public_data'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'VTUAVST':
        seq_home = '/nasdata/tracking/data/VTUAV/test/short-term'
        with open(join(join(seq_home, 'VTUAV-ST.txt')), 'r') as f:
            seq_list = f.read().splitlines()
    elif dataset_name == 'VTUAVLT':
        seq_home = '/nasdata/tracking/data/VTUAV/test/long-term'
