...
class mcitrack(object):
    def __init__(self, tracker_name='', para_name='',run_id=None,conf_thr=None,update_intervals=None,dataset_name='vot22'):
        # create tracker
        tracker_info = Tracker(tracker_name, para_name, dataset_name, run_id=run_id)
    ...
    def track(self, img_rgb):
        ...
        max_score = outputs['best_score'].cpu().numpy()[0][0]  #.max().cpu().numpy()
        return pred_bbox, max_score


def run_vot_exp(tracker_name, para_name, vis=False, out_conf=False, channel_type='color',run_id=None,conf_thr=None,update_intervals=None,dataset_name='vot22'):

    torch.set_num_threads(1)
    save_root = os.path.join('', para_name)
    if vis and (not os.path.exists(save_root)):
        os.mkdir(save_root)
    tracker = mcitrack(tracker_name=tracker_name, para_name=para_name,run_id=run_id,conf_thr=conf_thr,update_intervals=update_intervals,dataset_name=dataset_name)
