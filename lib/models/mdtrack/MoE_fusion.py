...
class BiMixtureOfAdapters(nn.Module):
    ...
    def forward(self, x,t,task_index):
        ...
        y_list = []
        total_loss = 0
        if type(task_index)  != list:
            task_index = task_index.tolist()
        for idx,task_index_v in enumerate(task_index):
            y_tmp = y[idx,:,:]
            y_tmp, aux_loss = self.MoA(y_tmp, task_index_v)
            total_loss += aux_loss
            y_list.append(y_tmp)
        ...
        # out_x = torch.add(out_x, self.modal_shifts[task_index_v*2+0], alpha=1)
        # out_t = torch.add(out_t, self.modal_shifts[task_index_v*2+1], alpha=1)
