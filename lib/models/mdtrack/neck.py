...
class merge_Injector(nn.Module):
    def __init__(self, d_model, n_heads=8,norm_layer=partial(nn.LayerNorm, eps=1e-6),  dropout=0.1,
                 init_values=0.):
        super().__init__()
        self.query_norm = norm_layer(d_model)
    ...
    def forward(self, query,feat,feat2):
            #query:l,b,d;feat:l,b,d
        def _inner_forward(query, feat, feat2):

            attn = self.attn(self.query_norm(query),
                             self.feat_norm(feat),self.feat_norm(feat2))[0]
            return query + self.gamma * attn
        query = _inner_forward(query, feat, feat2)
        return query
...
class Mamba_Neck(nn.Module):
    def __init__(self, in_channel=512,d_model=512,d_inner=1024,bias=False,n_layers=4,dt_rank=32,d_state=16,d_conv=3,dt_min=0.001,
        ...
        print(dt_scale,d_model,d_inner,dt_rank,d_state,bias,d_conv,conv_bias,dt_init,dt_max,dt_min,dt_init_floor,grad_ckpt)
...
def build_neck(cfg,encoder):
    ...
    print(in_channel,d_model,n_layers,d_state,grad_ckpt)
    return neck